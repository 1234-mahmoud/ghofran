...
const Ghofran = () => {
  ...
  return (
    <div
      className={`my-24 md:my-[200px] flex flex-col gap-14 md:gap-[56px] justify-center items-center px-4
    max-lg:my-[150px]
    `}
    >
      {/* Text Content */}
      <div className="flex flex-col justify-center items-center gap-6 md:gap-[24px] w-full max-w-[1135px] text-center">
        ...
        <p
          className={`${classes.gofran_text} font-bold text-base md:text-[20px] leading-relaxed md:leading-[40px] tracking-wide px-4 md:px-14 py-4`}
        >
      ...
      {/* Image Blocks */}
      <div
        className={`${classes.blocks} flex flex-col xl:flex-row gap-4 w-full max-w-[1744px] px-4`}
      >
        {/* Left Large Box */}
        <div
        ...
        {/* Right Grid Boxes */}
        <div
          className={`${classes.box2} w-full xl:max-w-[864px] flex flex-wrap gap-4`}
        >
          {imgs.map((i, index) => (
            <div
              key={index}
